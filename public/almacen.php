...
$sql = "SELECT a.ID_Almacen , u.Nombre_Usuario AS Encarga_Alm
FROM 
    almacen a
JOIN 
    usuario u ON a.Encarga_Alm = u.ID_Usuario";
...
<body>
    ...
    <h2 style="margin:0;">Almacen</h2>
    <button onclick="document.getElementById('formAgregar').style.display='block'" class="btn btn-add">Agregar Almacen</button>
</div>
        ...
        <form method="POST" action="../server/crud_almacen.php" style="display: flex; flex-wrap: wrap; gap: 10px;">
            <input type="text" name="encargado" placeholder="ID_Encargado" required style="flex:1; padding:8px;">
    ...
    if ($resultado && $resultado->num_rows > 0) {
        while ($fila = $resultado->fetch_assoc()) {
            echo "<tr>
                ...
                <td>" . htmlspecialchars($fila["Encarga_Alm"]) . "</td>
                <td>
                    <a class='btn btn-edit' href='../server/crud_almacen.php?id=" . $fila["ID_Almacen"] . "'>Modificar</a>
                    <form method='POST' action='../server/crud_almacen.php?id=" . $fila["ID_Almacen"] . "' style='display:inline;' onsubmit=\"return confirm('¿Seguro que deseas eliminar este envío?');\">
